...
"""Compare the public native and Triton softcap wrappers on a CUDA GPU."""
...
def _measure(profiler, fn):
    # The existing profiler warms up before recording per-invocation CUDA events.
    ...
    if not math.isfinite(median_ms) or median_ms <= 0:
        raise RuntimeError(f"invalid CUDA event latency: {median_ms}")
...
def _environment(device, args, triton_version, block_size):
    ...
    return {
        ...
        "nvidia_smi_name_driver": _command_output(
        ...
        "timer": "PerformanceProfiler CUDA events; median and sample standard deviation",
        "baseline": "eager NativeFinalLogitSoftcapOp on the same GPU; no torch.compile",
        "scope": "Public wrappers, including allocation and autograd dispatch; no CUDA graphs",
...
def _markdown(report):
    ...
    lines = [
        ...
        f"CUDA runtime: {env['cuda_runtime']}.",
        ...
        "",
        "Contiguous inputs; median CUDA event latency for public wrappers, including "
...
def run_benchmark(args):
    ...
    if device.type != "cuda" or not torch.cuda.is_available() or torch.version.hip is not None:
        raise RuntimeError("This benchmark requires an NVIDIA CUDA GPU; no CPU fallback is timed")
    ...
    with torch.cuda.device(device):
        ...
        report = {
            "environment": _environment(device, args, triton.__version__, _BLOCK),
            "results": [],
        }
